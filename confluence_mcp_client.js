...
function confluenceHtmlToMarkdown(html) {
  ...
  // Vietnamese diacritics (common in Confluence storage format)
  const namedEntities = {
    agrave: 'à',
    aacute: 'á',
    atilde: 'ã',
  ...
  // Numeric entities: &#NNN; → character
  md = md.replace(/&#(\d+);/g, (_m, code) => String.fromCharCode(parseInt(code, 10)))
  // Hex entities: &#xHHH; → character
  md = md.replace(/&#x([0-9a-fA-F]+);/g, (_m, hex) => String.fromCharCode(parseInt(hex, 16)))
...
export class ConfluenceMCPClient {
   ...
   */
  async getMultiplePagesAsMarkdown(urls) {
    const pages = []
    const sections = []

    for (const url of urls) {
      const result = await this.getPageAsMarkdown(url)
      pages.push({ title: result.title, pageId: result.pageId, url: result.url })
